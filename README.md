...
- 新增“@管理员 + 关键词词条”专用规则：当群聊消息 `@` 到管理员 QQ 且文本命中某条关键词词条时，可按该词条独立配置的概率、冷却时间和提示词触发语音回复。例如 `[At:995051631] 摸摸`。
...
- `admin_mention_keyword_voice_entries`：推荐使用的新词条配置。每行一个词条，格式是 `关键词|概率|冷却秒数|提示词`。例如：`摸摸|100|30|请温柔地用语音回应对方的撒娇请求。`
- `admin_mention_keyword_voice_entries`：仅在群聊里“`@管理员 + 命中关键词`”时生效。多个词条同时命中时，优先更长的关键词；长度相同则按配置顺序。
- `admin_mention_keyword_voice_entries`：词条命中且概率成功后，会把该词条的提示词注入到本次请求中；插件会自动补上“本次回复需要包含语音消息”。
- `admin_mention_keyword_voice_keywords` / `admin_mention_keyword_voice_probability` / `admin_mention_keyword_voice_prompt`：旧版兼容配置。只有当 `admin_mention_keyword_voice_entries` 为空时才会回退使用。
...
- `auto_japanese_voice_translate_prompt`：将中文回复转换成更连贯的混合输出时使用的提示词。推荐让模型直接输出带 `<tts>` 的完整格式；如果只输出纯日语文本，插件也会自动补成语音并在后面附上原中文。

词条格式示例：
`摸摸|100|30|请温柔地用语音回应对方的撒娇请求。`
`抱抱|80|60|请用带安抚感的语音回应，并多一些陪伴语气。`
...
- 如果你希望只有像“`@管理员 摸摸`”“`@管理员 抱抱`”这类消息触发特殊语音，建议直接配置 `admin_mention_keyword_voice_entries`，按词条分别设置概率、冷却时间和提示词。
